...
import { createAndFundEscrow, startEscrow, submitEscrow, approveEscrow } from './contracts/localEscrow';
...
function App() {
  ...
  async function startEscrowDemo() {
    try {
      ...
      setLocalContractId(result.contractId);
      setContractStep('Funded');
  ...
  async function runContractAction(action: 'start' | 'submit' | 'approve') {
    if (!localContractId) return setNotice('Fund a local contract first.');
    try {
      if (action === 'start') await startEscrow(localContractId);
      if (action === 'submit') await submitEscrow(localContractId, `evidence-${Date.now()}`);
      if (action === 'approve') await approveEscrow(localContractId);
      const labels = { start: 'Work started', submit: 'Evidence submitted', approve: 'Milestone approved and paid' };
      setContractStep(labels[action]);
      setNotice(`${labels[action]} on Hardhat.`);
    } catch (error: any) {
      setNotice(error?.shortMessage || error?.message || 'Contract action failed.');
    }
  }
  ...
  return <div className="app-shell">
    ...
    {!isAuthenticated ? <main className="landing-main">
      ...
      <section className="content"><div className="content-top"><div><div className="eyebrow">{userProfile?.role === 'recruiter' ? 'RECRUITER WORKSPACE' : 'DEVELOPER WORKSPACE'}</div><h1>{activeView === 'overview' ? `Good to see you, ${userProfile?.name?.split(' ')[0]}.` : activeView[0].toUpperCase() + activeView.slice(1)}</h1><p className="muted">{activeView === 'overview' ? 'Your professional graph, at a glance.' : 'Keep the important work moving.'}</p></div>{userProfile?.role === 'recruiter' && <button className="primary-button compact" onClick={() => setShowProjectForm(true)}>+ Publish project</button>}</div>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>×</button></div>}
        ...
        {activeView === 'contracts' && <div className="panel full-panel"><div className="panel-heading"><div><span className="eyebrow">MILESTONE ESCROW</span><h2>Your contracts</h2></div></div>{milestones.length ? milestones.map((milestone) => <div className="contract-row" key={milestone.id}><div className="contract-status"><span className={`status-pill ${milestone.status}`}>{milestone.status}</span><strong>{milestone.title}</strong><p>{milestone.description}</p></div><div className="contract-amount"><strong>{milestone.amount_eth} ETH</strong><span>Milestone amount</span></div><div className="contract-actions"><button className="outline-button" onClick={startEscrowDemo}>Fund on Hardhat</button>{localContractId && <><button className="outline-button" onClick={() => runContractAction('start')}>Start work</button><button className="outline-button" onClick={() => runContractAction('submit')}>Submit evidence</button><button className="primary-button compact" onClick={() => runContractAction('approve')}>Approve & release</button></>}</div></div>) : <><div className="local-contract-card"><span className="eyebrow">LOCAL HARDHAT DEMO</span><h3>{localContractId ? `Contract #${localContractId}` : 'No local contract yet'}</h3><p>{contractStep}. Use the actions below to demonstrate the complete milestone lifecycle.</p><div className="contract-actions"><button className="primary-button compact" onClick={startEscrowDemo}>Create & fund contract</button>{localContractId && <><button className="outline-button" onClick={() => runContractAction('start')}>Start work</button><button className="outline-button" onClick={() => runContractAction('submit')}>Submit evidence</button><button className="primary-button compact" onClick={() => runContractAction('approve')}>Approve & release</button></>}</div></div><EmptyState text="Your backend milestones will appear here." /></>}</div>}
